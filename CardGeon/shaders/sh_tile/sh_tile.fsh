...
void main()
{
	...
	//
	vec4 topColor = texture2D( gm_BaseTexture, virtualCoord ) *  v_vColour;
	...
	//vec4 botColor = texture2D( gm_BaseTexture, virtualCoord ) *  v_vColour;
	
	//float bot = float(botColor.r > 0.9);	
	//if (Coord.x < 0.1 || Coord.x > 0.9) col = vec4(1.0, 0.0, 0.0, 1.0);
	
	gl_FragColor = clamp(topColor , 0.0, 1.0);
}