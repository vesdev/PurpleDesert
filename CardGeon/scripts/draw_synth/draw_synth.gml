...
//	clipping_mask : surface_create(380, 380),
		...
		var horizon_x = -50;				
			...
			for (var i = -0; i < 10; i += 1)
			{
				...
				//custom shape script
				//draw_shape(horizon_x + 1, round(horizon_y), 3 , ang, radius, col, 1,0,thickness);
			}
	
			draw_set_colour(c_black);
			draw_rectangle(-o_game.camera.width*10, horizon_y, 500, 500, false);
			draw_set_colour(C_PURPLE);
			    ...
			    for (var i = -lines_+1; i < lines_; i += 1)
				{
					...
					draw_line(lines_/2 * i - i*5, surf_height, horizon_x + i*5 - 5, horizon_y );
				}
				...
				for (var i = 0; i <30; i += 1)
				{
					...
					var yy_ = horizon_y + (((spd + i/30)%1) );
					draw_line( -o_game.camera.width*5 , yy_, surf_width , yy_);
				}
